...
void loop() {
  ...
  if (now - lastMsg > msgPeriod) {
    ...
    DynamicJsonDocument resp(256);
    resp["Cantidad_Autos_Disponibles"] = CantidadAutosDisp; //temperature;  //Agrega el dato al Json, ej: "temperature": 21.5
       //Agrega el dato al Json, ej: "humidituy: 75.0
    char buffer[256];
    serializeJson(resp, buffer);
  ...
  CantidadAutosDisp= DigitalRead(Sensor1)+DigitalRead(Sensor2)+DigitalRead(Sensor3)+DigitalRead(Sensor4)+DigitalRead(Sensor5)+DigitalRead(Sensor6);
}
```
